...
#' Google.}
#'  \item{name}{a string -  The unique name of the AdRule. This attribute is required to create an ad
...
#'    }
#'   }
#'  \item{videoMidrollFrequency}{a string -  The mid-roll frequency of this ad rule slot for video ads. This attribute is
...
#'  \item{videoMidrollFrequency}{a string (inherited from BaseAdRuleSlot) -  The mid-roll frequency of this ad rule slot for video ads. This attribute is
#' required for mid-rolls, but if MidrollFrequencyType is set to
#' MidrollFrequencyType NONE, this value should be ignored. For example, if
#' this slot has a frequency type of MidrollFrequencyType EVERY_N_SECONDS and
#' videoMidrollFrequency = "60", this would mean " play a mid-roll every 60
...
dfp_createAdRules <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['createAdRulesResponse']])$rval
...
dfp_getAdRulesByStatement <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getAdRulesByStatementResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @export
dfp_performAdRuleAction <- function(){

  request_body <- make_request_body(service='AdRuleService', root_name='performAdRuleAction', data=NULL)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['performAdRuleActionResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
dfp_updateAdRules <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['updateAdRulesResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
