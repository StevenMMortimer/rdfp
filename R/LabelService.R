...
#'  \item{name}{a string -  Name of the Label. This is value is required to create a label and has a
#' maximum length of 127 characters.}
#'  \item{description}{a string -  A description of the label. This value is optional and its maximum length is
#' 255 characters.}
...
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a createLabelsResponse
#' @export
dfp_createLabels <- function(request_data){

  request_body <- make_request_body(service='LabelService', root_name='createLabels', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['createLabelsResponse']])$rval
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a getLabelsByStatementResponse
#' @export
dfp_getLabelsByStatement <- function(request_data){

  request_body <- make_request_body(service='LabelService', root_name='getLabelsByStatement', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getLabelsByStatementResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a updateLabelsResponse
#' @export
dfp_updateLabels <- function(request_data){

  request_body <- make_request_body(service='LabelService', root_name='updateLabels', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['updateLabelsResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
