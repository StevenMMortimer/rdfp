...
dfp_ReconciliationOrderReportService_object_factory <- function(obj_type, obj_data){
  ...
  if(obj_type=='ReconciliationOrderReportPage'){
...
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a getReconciliationOrderReportsByStatementResponse
#' @export
dfp_getReconciliationOrderReportsByStatement <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getReconciliationOrderReportsByStatementResponse']])$rval
...
#' @export
dfp_performReconciliationOrderReportAction <- function(){

  request_body <- make_request_body(service='ReconciliationOrderReportService', root_name='performReconciliationOrderReportAction', data=NULL)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['performReconciliationOrderReportActionResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a updateReconciliationOrderReportsResponse
#' @export
dfp_updateReconciliationOrderReports <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['updateReconciliationOrderReportsResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
