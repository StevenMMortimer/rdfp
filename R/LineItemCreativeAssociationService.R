...
#'  \item{destinationUrl}{a string -  Overrides the value set for HasDestinationUrlCreative destinationUrl. This
...
#'  \item{targetingName}{a string -  Specifies CreativeTargeting for this line item creative association. This
...
dfp_LineItemCreativeAssociationService_object_factory <- function(obj_type, obj_data){
  ...
  if(obj_type=='CreativePreviewError'){
    stopifnot(all(sampled_names %in% c('reason')))
  }
  ...
  if(obj_type=='LineItemCreativeAssociationOperationError'){
    stopifnot(all(sampled_names %in% c('reason')))
  }

  if(obj_type=='LineItemCreativeAssociationPage'){
...
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a createLineItemCreativeAssociationsResponse
#' @export
dfp_createLineItemCreativeAssociations <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['createLineItemCreativeAssociationsResponse']])$rval
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a getLineItemCreativeAssociationsByStatementResponse
#' @export
dfp_getLineItemCreativeAssociationsByStatement <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getLineItemCreativeAssociationsByStatementResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
dfp_getPreviewUrl <- function(){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getPreviewUrlResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @export
dfp_performLineItemCreativeAssociationAction <- function(){

  request_body <- make_request_body(service='LineItemCreativeAssociationService', root_name='performLineItemCreativeAssociationAction', data=NULL)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['performLineItemCreativeAssociationActionResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a updateLineItemCreativeAssociationsResponse
#' @export
dfp_updateLineItemCreativeAssociations <- function(request_data){
  ...
  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['updateLineItemCreativeAssociationsResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
