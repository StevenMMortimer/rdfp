...
#' @seealso \href{https://developers.google.com/doubleclick-publishers/docs/reference/v201711/CreativeTemplateService#getCreativeTemplatesByStatement}{Google Documentation for getCreativeTemplatesByStatement}
#' 
...
dfp_getCreativeTemplatesByStatement <- function(request_data, as_df=FALSE, verbose=FALSE){
 request_body <- make_request_body(service='CreativeTemplateService', root_name='getCreativeTemplatesByStatement', data=request_data)
  ...
  response <- NULL
  response <- try(xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getCreativeTemplatesByStatementResponse']]), silent=T)
