...
#' \describe{
#'  \item{labelName}{a string -  Represents the column's name.}
#' }
#' 
#' \strong{CommonError}
#' 
...
dfp_select <- function(){
  ...
  request_body <- make_request_body(service='PublisherQueryLanguageService', root_name='select', data=NULL)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['selectResponse']])$rval
