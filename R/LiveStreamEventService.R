...
#' assigned by Google.}
#'  \item{name}{a string -  The name of the LiveStreamEvent. This value is required to create a live
#' stream event and has a maximum length of 255 characters.}
#'  \item{description}{a string -  Additional notes to annotate the event with. This attribute is optional and
...
#'  \item{contentUrls}{a string -  The list of URLs pointing to the live stream content in Content Delivery
#' Network. This attribute is required.}
#'  \item{adTags}{a string -  The list of DFP ad tag URLs generated by DFP trafficking workflow that are
#' associated with this live stream event. This attribute is required.}
#'  \item{liveStreamEventCode}{a string -  This code is used in constructing a live stream event master playlist URL.
...
dfp_LiveStreamEventService_object_factory <- function(obj_type, obj_data){
  ...
  if(obj_type=='LiveStreamEventActionError'){
    stopifnot(all(sampled_names %in% c('reason')))
  }

  if(obj_type=='LiveStreamEventDateTimeError'){
    stopifnot(all(sampled_names %in% c('reason')))
  }

  if(obj_type=='LiveStreamEventPage'){
...
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a createLiveStreamEventsResponse
#' @export
dfp_createLiveStreamEvents <- function(request_data){

  request_body <- make_request_body(service='LiveStreamEventService', root_name='createLiveStreamEvents', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['createLiveStreamEventsResponse']])$rval
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a getLiveStreamEventsByStatementResponse
#' @export
dfp_getLiveStreamEventsByStatement <- function(request_data){

  request_body <- make_request_body(service='LiveStreamEventService', root_name='getLiveStreamEventsByStatement', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['getLiveStreamEventsByStatementResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @export
dfp_performLiveStreamEventAction <- function(){

  request_body <- make_request_body(service='LiveStreamEventService', root_name='performLiveStreamEventAction', data=NULL)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['performLiveStreamEventActionResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
...
#' @param request_data a \code{list} or \code{data.frame} of data elements
#' to be formatted for a SOAP request (XML format, but passed as character string)#' @return a \code{list} containing all the elements of a updateLiveStreamEventsResponse
#' @export
dfp_updateLiveStreamEvents <- function(request_data){

  request_body <- make_request_body(service='LiveStreamEventService', root_name='updateLiveStreamEvents', data=request_data)
  request <- build_soap_request(body = request_body)

  response <- xmlChildren(xmlChildren(xmlChildren(xmlRoot(request))$Body)[['updateLiveStreamEventsResponse']])$rval
  response <- if(is.null(response)) NULL else xmlToList(response)
  return(response)
}
#' 
