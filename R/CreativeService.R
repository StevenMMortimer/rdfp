...
dfp_createCreatives <- function(request_data, as_df=TRUE, verbose=FALSE){
  ...
  result <- parse_soap_response(httr_response=response, resp_element='createCreativesResponse', as_df=as_df)
  return(result)
}
#' 
...
dfp_getCreativesByStatement <- function(request_data, as_df=TRUE, verbose=FALSE){
  ...
  response <- execute_soap_request(body=request_body, verbose=verbose)
  result <- parse_soap_response(httr_response=response, resp_element='getCreativesByStatementResponse', as_df=as_df)
  return(result)
}
#' 
#' updateCreatives
...
dfp_updateCreatives <- function(request_data, as_df=TRUE, verbose=FALSE){
  ...
  response <- execute_soap_request(body=request_body, verbose=verbose)
  result <- parse_soap_response(httr_response=response, resp_element='updateCreativesResponse', as_df=as_df)
  return(result)
}
#' 
